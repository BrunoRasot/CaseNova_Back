...
const getUsuarios = async (req, res) => {
  try {
    ...
    console.error('ERROR_GET_USUARIOS:', error);
...
const updateEstadoUsuario = async (req, res) => {
  ...
  try {
    const { rows: target } = await pool.query('SELECT id, nombre FROM usuarios WHERE id = $1 LIMIT 1', [id]);
    if (target.length === 0) {
    ...
    console.error('ERROR_UPDATE_ESTADO_USUARIO:', error);
...
const deleteUsuario = async (req, res) => {
  ...
  try {
    ...
    const { rows: ventas } = await pool.query('SELECT COUNT(*) AS total FROM ventas WHERE usuario_id = $1', [id]);
    ...
    try {
      await pool.query('UPDATE logs_acceso SET usuario_id = NULL WHERE usuario_id = $1', [id]);
    } catch (logsError) {
